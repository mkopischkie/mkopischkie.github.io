...
Initially, the algorithm computs the dot product between two vectors to gain insight into how similar the vectors are related. For example, if the dot product between two vectors is zero, the two vectors are perfectly orthogonal, and if the dot product is one, the vectors are perfectly parallel. The dot product is defined as $x \cdot y = \left|\left|x\right|\right| \left|\left|y\right|\right| cos(\theta)$ , where $\theta$ is the angle measured between the two vectors. 
